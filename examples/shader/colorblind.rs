...
pub const FIRST_PASS_DRIVER: &str = "post_process_pass_driver";
...
fn main() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_node_edge(node::MAIN_PASS_DRIVER, FIRST_PASS_DRIVER)
        .unwrap();
    app.run();
}
...
fn setup(
    ...
    // Post processing pass 2d quad, with material containing the rendered first pass texture, with a custom shader.
    commands
...
impl Material2d for PostProcessingMaterial {
    ...
    fn fragment_shader(asset_server: &AssetServer) -> Option<Handle<Shader>> {
        ...
        Some(asset_server.load("shaders/colorblind.wgsl"))
    }
}
